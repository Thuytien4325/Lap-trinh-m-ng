document.addEventListener('DOMContentLoaded', () => {
  ...
  if (loginForm) {
    loginForm.addEventListener('submit', async function (event) {
      ...
      // Tạo form-urlencoded data
      const formData = new URLSearchParams();
      formData.append('username', username);
      formData.append('password', password);
      formData.append('grant_type', 'password'); // Cần thiết cho OAuth2 Password flow
      ...
      try {
        ...
        if (response.ok && result.access_token) {
          ...
          }, 1500); // Cho người dùng thấy toast rồi mới chuyển trang
        } else {
          loginError.textContent = result.detail || 'Sai tài khoản hoặc mật khẩu!';
          toast({
